...
| `pi-ext/*.ts` | Pi extensions, loaded by `pi` itself via `settings.json`'s `extensions` array (written by `config.sh:_pi_extensions`). `codebox-guard.ts` protects credentials and Docker; `codebox-mcp.ts` is an MCP stdio client; `subagent/` vendors Pi's model-pinned isolated-task delegation example. Test with `scripts/verify-pi-extensions.sh` |
