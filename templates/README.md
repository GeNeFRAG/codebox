...
| `flowcode.mcp.json.template` | `/root/.config/flowcode/config.json` | FlowCode |
...
2. Add `${MY_VAR}` to the `envsubst` variable list in `lib/config.sh` (function `_generate_config`, `_generate_claude_code_config`, or `_generate_flowcode_config`).
