...
| `scripts/verify-pi-extensions.sh` | Loads both Pi extensions through pi's own jiti loader and asserts ~45 block/allow verdicts plus a live MCP discover-call-cache round trip. Run in-container after touching `lib/pi-ext/` or `lib/guard-bin/docker`; `SKIP_MCP=1` skips the spawning half |
